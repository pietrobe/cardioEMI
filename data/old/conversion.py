...
def main():
    ...
    # Ensure boundary_map contains only pure Python types
    boundary_map_pure = {int(k): {int(x) for x in v} for k, v in boundary_map.items()}  

    # Save the pickle file
    with open(f"{prefix_out}.pickle", "wb") as f:
        pickle.dump(boundary_map_pure, f, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    """with open("old_pickle.pickle", "rb") as f:
    data = pickle.load(f)
    ...
    with open("new_pickle.pickle", "wb") as f:
